...
def notify_tenant_nearing_due():
    get_today = datetime.datetime.utcnow()
    ...
    for inst in rent_instances:
        ...
        subject = "UPCOMING RENT PAYMENT NOTIFICATION FOR {0} RENTAL UNIT {1}".format(
            inst.tenant.full_name, inst.unit.unit_number
            )
        html_content = 'utilities_and_rent/mails/send_nearing_due_notification.html'
        html_message = render_to_string(html_content, {'data':inst,'today':get_today,})
...
def start():
    ...
    scheduler.add_job(notify_tenant_nearing_due, 'interval', minutes=11520) # every 8 days, TODO: reset to 2880 | 48 hour schedule
    scheduler.start()